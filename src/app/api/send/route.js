...
export async function POST(req) {
    try {
        ...
        if (lastRequest && (now - lastRequest) < COOLDOWN_PERIOD) {
            const remainingTime = Math.ceil((COOLDOWN_PERIOD - (now - lastRequest)) / 1000 / 60);
        ...
        if (requestLog.size > 10000) { // Limit wielkości mapy
            const oldEntries = Array.from(requestLog.entries())
                .filter(([_, timestamp]) => now - timestamp > COOLDOWN_PERIOD);
